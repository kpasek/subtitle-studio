...
DEFAULT_FILTERS = {
    ...
    "acompressor": {
        "enabled": False,
        "params": "threshold=-18dB:ratio=2:attack=5:release=120:makeup=2",
    },
...
DEFAULT_TTS_CONCURRENCY = {
    ...
    "generators": {
        "xtts": {"enabled": False, "threads": 4},
        "elevenlabs": {"enabled": True, "threads": 4},
...
class SettingsWindow(ctk.CTkToplevel):
    ...
    def __init__(
        self, master: "SubtitleStudioApp", torch_installed: bool, mode: str = "global"
    ):
        ...
        ctk.CTkButton(btn_frame, text="Anuluj", command=self.destroy).pack(
            side="right", padx=(10, 0)
        )
        ctk.CTkButton(
            btn_frame, text="Zapisz i Zamknij", command=self.save_and_close
        ).pack(side="right")
    ...
    def _create_global_tab(self, frame: ctk.CTkScrollableFrame):
        ...
        # Start Directory
        ctk.CTkLabel(tab_general, text="Startowy katalog:").grid(
            row=0, column=0, sticky="w", padx=10, pady=10
        )
        self.start_dir_var = tk.StringVar(
            value=self.master.global_config.get("start_directory", "")
        )
        ctk.CTkEntry(tab_general, textvariable=self.start_dir_var).grid(
            row=0, column=1, sticky="ew", padx=(0, 10), pady=10
        )
        ctk.CTkButton(
            tab_general, text="...", width=40, command=self.select_start_dir
        ).grid(row=0, column=2, sticky="e", padx=10, pady=10)

        # Workers
        ctk.CTkLabel(tab_general, text="Procesy weryfikacji:").grid(
        ...
        self.verification_workers_var = tk.StringVar(
            value=self.master.global_config.get(
                "verification_workers",
                self.master.global_config.get("conversion_workers", default_workers),
            )
        )
        entry_workers = ctk.CTkEntry(
            tab_general, textvariable=self.verification_workers_var
        )
        entry_workers.grid(row=1, column=1, sticky="ew", padx=(0, 10), pady=10)
        CreateToolTip(
            entry_workers,
            f"Liczba procesów do weryfikacji audio (max: {cpu_count}).",
            wraplength=300,
        )

        # Audio Format
        ctk.CTkLabel(tab_general, text="Format wyjściowy audio:").grid(
        ...
        ctk.CTkOptionMenu(
            tab_general, variable=self.audio_format_var, values=["ogg", "mp3"]
        ).grid(row=2, column=1, sticky="w", padx=(0, 10), pady=10)
        ctk.CTkLabel(tab_general, text="(Format plików w folderze /ready)").grid(
        ...
        ctk.CTkLabel(t_xtts, text="URL serwera API:").grid(
            row=0, column=0, sticky="w", padx=10, pady=10
        )
        self.local_api_url_var = tk.StringVar(
            value=self.master.global_config.get(
                "local_api_url", "http://127.0.0.1:8001"
            )
        )
        ctk.CTkEntry(t_xtts, textvariable=self.local_api_url_var).grid(
            row=0, column=1, columnspan=2, sticky="ew", padx=10, pady=10
        )

        ctk.CTkLabel(t_xtts, text="Ścieżka głosu XTTS (.wav):").grid(
            row=1, column=0, sticky="w", padx=10, pady=10
        )
        self.xtts_voice_path_var = tk.StringVar(
            value=self.master.global_config.get("xtts_voice_path", "")
        )
        ctk.CTkEntry(t_xtts, textvariable=self.xtts_voice_path_var).grid(
            row=1, column=1, sticky="ew", padx=(0, 10), pady=10
        )
        ctk.CTkButton(
            t_xtts, text="...", width=40, command=self.select_voice_file
        ...
        ctk.CTkLabel(t_google, text="Credentials (.json):").grid(
            row=0, column=0, sticky="w", padx=10, pady=10
        )
        self.gcp_creds_var = tk.StringVar(
            value=self.master.global_config.get("google_credentials_path", "")
        )
        ctk.CTkEntry(t_google, textvariable=self.gcp_creds_var).grid(
            row=0, column=1, sticky="ew", padx=(0, 10), pady=10
        )
        ctk.CTkButton(
            t_google, text="...", width=40, command=self.select_gcp_creds
        ).grid(row=0, column=2, sticky="e", padx=10, pady=10)

        ctk.CTkLabel(t_google, text="Nazwa głosu:").grid(
            row=1, column=0, sticky="w", padx=10, pady=10
        )
        self.gcp_voice_name_var = tk.StringVar(
            value=self.master.global_config.get("google_voice_name", "pl-PL-Wavenet-B")
        )
        ctk.CTkEntry(t_google, textvariable=self.gcp_voice_name_var).grid(
            row=1, column=1, sticky="ew", padx=10, pady=10
        )
        ...
        ctk.CTkLabel(t_piper, text="Ścieżka modelu Piper (.onnx):").grid(
            row=0, column=0, sticky="w", padx=10, pady=10
        )
        self.piper_model_path_var = tk.StringVar(
            value=self.master.global_config.get("piper_model_path", "")
        )
        ctk.CTkEntry(t_piper, textvariable=self.piper_model_path_var).grid(
            row=0, column=1, sticky="ew", padx=(0, 10), pady=10
        )
        ctk.CTkButton(
            t_piper, text="...", width=40, command=self.select_model_file
        ).grid(row=0, column=2, sticky="e", padx=10, pady=10)

        # TeamSP
        t_teamsp = tts_inner_tabs.add("TeamSP")
        t_teamsp.grid_columnconfigure(1, weight=1)
        ctk.CTkLabel(t_teamsp, text="ID głosu TeamSP:").grid(
            row=0, column=0, sticky="w", padx=10, pady=10
        )
        self.teamsp_voice_id_var = tk.StringVar(
            value=self.master.global_config.get("teamsp_voice_id", "")
        )
        ctk.CTkEntry(t_teamsp, textvariable=self.teamsp_voice_id_var).grid(
            row=0, column=1, sticky="ew", padx=10, pady=10
        )
        CreateToolTip(t_teamsp, "Identyfikator (string) głosu dla generatora TeamSP.")

        # Współbieżność TTS
        t_concurrency = tts_inner_tabs.add("Współbieżność")
        self._create_tts_concurrency_tab(t_concurrency)

        # --- TAB: SI ---
        tab_ai.grid_columnconfigure(1, weight=1)
        ctk.CTkLabel(tab_ai, text="Url serwera Ollama:").grid(
            row=0, column=0, sticky="w", padx=10, pady=10
        )
        self.ollama_url_var = tk.StringVar(
            value=self.master.global_config.get("ollama_url", "http://localhost:11434")
        )
        ctk.CTkEntry(tab_ai, textvariable=self.ollama_url_var).grid(
            row=0, column=1, sticky="ew", padx=10, pady=10
        )
        CreateToolTip(tab_ai, "Adres API Ollama (domyślnie http://localhost:11434)")

        ctk.CTkLabel(tab_ai, text="Model Ollama:").grid(
            row=1, column=0, sticky="w", padx=10, pady=10
        )
        self.ollama_model_var = tk.StringVar(
            value=self.master.global_config.get("ollama_model", "gemma2:2b")
        )
        ctk.CTkEntry(tab_ai, textvariable=self.ollama_model_var).grid(
            row=1, column=1, sticky="ew", padx=10, pady=10
        )
        CreateToolTip(
            tab_ai, "Nazwa modelu do użycia (musi być zainstalowana w Ollama)"
        )
        ...
        for key, default_config in DEFAULT_FILTERS.items():
            ...
            ctk.CTkEntry(tab_filters, textvariable=p_var).grid(
                row=current_row, column=1, sticky="ew", padx=10, pady=5
            )
            ctk.CTkCheckBox(tab_filters, text="Włącz", variable=e_var).grid(
                row=current_row, column=2, sticky="e", padx=10, pady=5
            )
            current_row += 1

        # --- TAB: Wygląd ---
        # Appearance Mode
        ctk.CTkLabel(tab_theme, text="Wygląd (Jasny/Ciemny/System):").pack(
        ...
        # Color Theme
        ctk.CTkLabel(tab_theme, text="Paleta kolorów:").pack(
            anchor="w", padx=10, pady=(10, 0)
        )
    ...
    def _create_project_tab(self, frame: ctk.CTkFrame):
        ...
        self.btn_browse_xtts_voice_project = ctk.CTkButton(
            frame,
            text="...",
            width=30,
            command=lambda: self.select_voice_file(self.xtts_voice_project_path_var),
        )
        self.btn_browse_xtts_voice_project.grid(row=4, column=2, padx=10, pady=5)

        CreateToolTip(
            self.ent_xtts_voice_project,
        ...
        self.btn_browse_piper_model_project = ctk.CTkButton(
            frame,
            text="...",
            width=30,
            command=lambda: self.select_model_file(self.piper_model_project_path_var),
        )
        self.btn_browse_piper_model_project.grid(row=5, column=2, padx=10, pady=5)

        CreateToolTip(
            self.ent_piper_model_project,
        ...
        CreateToolTip(
            self.ent_teamsp_voice_project,
            "Opcjonalne: Nadpisz globalny ID głosu TeamSP tylko dla tego projektu.",
        )

    def _create_tts_concurrency_tab(self, frame: ctk.CTkFrame):
        """Creates the TTS concurrency settings tab."""
        frame.grid_columnconfigure(1, weight=1)
        frame.grid_columnconfigure(2, weight=1)

        tts_concurrency = self.master.global_config.get(
        ...
        ctk.CTkLabel(frame, text="Globalny limit wątków:", font=("", 12, "bold")).grid(
            row=0, column=0, sticky="w", padx=10, pady=(15, 5), columnspan=3
        )
        self.global_max_threads_var = tk.StringVar(value=str(global_max))
        entry_global_max = ctk.CTkEntry(
            frame, textvariable=self.global_max_threads_var, width=100
        )
        entry_global_max.grid(row=1, column=0, sticky="w", padx=10, pady=(0, 15))
        CreateToolTip(
            entry_global_max, "Maksymalna liczba wątków dla wszystkich generatorów TTS."
        )

        ctk.CTkLabel(frame, text="Generator", font=("", 10, "bold")).grid(
            row=2, column=0, sticky="w", padx=10, pady=(10, 5)
        )
        ctk.CTkLabel(frame, text="Włącz", font=("", 10, "bold")).grid(
            row=2, column=1, pady=(10, 5)
        )
        ctk.CTkLabel(frame, text="Wątki", font=("", 10, "bold")).grid(
            row=2, column=2, pady=(10, 5)
        )

        self.concurrency_vars = {}
        for i, (gen_id, gen_name) in enumerate(TTS_GENERATOR_NAMES.items(), start=3):
            gen_config = generators.get(
                gen_id, DEFAULT_TTS_CONCURRENCY["generators"].get(gen_id)
            )
            enabled = gen_config.get("enabled", False) if gen_config else False
            threads = gen_config.get("threads", 4) if gen_config else 4

            ctk.CTkLabel(frame, text=gen_name).grid(
                row=i, column=0, sticky="w", padx=10, pady=5
            )

            enabled_var = tk.BooleanVar(value=enabled)
            threads_var = tk.StringVar(value=str(threads))

            checkbox = ctk.CTkCheckBox(frame, text="", variable=enabled_var, width=50)
            checkbox.grid(row=i, column=1, pady=5)

            threads_entry = ctk.CTkEntry(frame, textvariable=threads_var, width=80)
            threads_entry.grid(row=i, column=2, sticky="e", padx=10, pady=5)

            self.concurrency_vars[gen_id] = {
                "enabled": enabled_var,
                "threads": threads_var,
                "checkbox": checkbox,
    ...
    def select_model_file(self, ent_path_var=None):
        """Opens dialog to select XTTS voice file."""
        path = filedialog.askopenfilename(
            title="Wybierz plik głosu .onnx",
            filetypes=[("Model", "*.onnx")],
            initialdir=self._get_initial_dir(),
            parent=self,
        )
        if path:
    ...
    def select_gcp_creds(self):
        ...
        path = filedialog.askopenfilename(
            title="Wybierz credentials .json",
            filetypes=[("JSON", "*.json")],
            initialdir=self._get_initial_dir(),
            parent=self,
        )
    ...
    def _save_global_settings(self):
        ...
        try:
            ...
            filters_data = {
                key: {"enabled": en_var.get(), "params": par_var.get()}
            ...
            try:
                global_max = max(1, int(self.global_max_threads_var.get()))
            except ValueError:
                global_max = 16

            generators_concurrency = {}
            for gen_id in TTS_GENERATOR_NAMES:
                if gen_id in self.concurrency_vars:
                    vars_dict = self.concurrency_vars[gen_id]
                    try:
                        threads = max(1, int(vars_dict["threads"].get()))
                    except ValueError:
                        threads = 4
                    generators_concurrency[gen_id] = {
                        "enabled": vars_dict["enabled"].get(),
                        "threads": threads,
                    }
                else:
                    generators_concurrency[gen_id] = {"enabled": False, "threads": 4}

            global_data["tts_concurrency"] = {
                "global_max_threads": global_max,
                "generators": generators_concurrency,
            }
            ...
            Logger.info(
                "Wyczyszczono cache modelu TTS z powodu zapisu ustawień globalnych."
            )
